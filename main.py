...
if page == "Home":
    ...
    if not df_estudo.empty:
        ...
        for col in ['paginas', 'humor', 'tipo_estudo', 'dia_cronograma']:
            if col not in df_estudo.columns:
                df_estudo[col] = 0 if col in ['paginas', 'dia_cronograma'] else "N/A"
    ...
    # Lógica para descobrir qual foi o último dia estudado no ciclo
    ultimo_dia = "Nenhum"
    if not df_estudo.empty and 'dia_cronograma' in df_estudo.columns:
        dias_validos = pd.to_numeric(df_estudo['dia_cronograma'], errors='coerce').dropna()
        if not dias_validos.empty and dias_validos.iloc[-1] > 0:
            ultimo_dia = f"Dia {int(dias_validos.iloc[-1]):02d}"

    # Agora são 5 colunas para caber o novo card
    ...
    with c3: st.markdown(f'<div class="card"><div class="title">Ofensiva</div><div class="value">🔥 {streak_atual} Dias</div></div>', unsafe_allow_html=True)
    ...
    with c5: st.markdown(f'<div class="card" style="border-top: 4px solid #3ec6a8;"><div class="title">Último Ciclo</div><div class="value">📍 {ultimo_dia}</div></div>', unsafe_allow_html=True)
    ...
    if not df_estudo.empty:
        ...
        fig_radar.update_layout(
            ...
            shapes=[
                dict(type="circle", xref="polar", yref="polar", x0=-100, y0=-100, x1=100, y1=100, fillcolor="#1a531a", opacity=0.4, line_width=0, layer="below"),
                dict(type="circle", xref="polar", yref="polar", x0=-90, y0=-90, x1=90, y1=90, fillcolor="#2ecc71", opacity=0.4, line_width=0, layer="below"),
                dict(type="circle", xref="polar", yref="polar", x0=-80, y0=-80, x1=80, y1=80, fillcolor="#f1c40f", opacity=0.4, line_width=0, layer="below"),
                dict(type="circle", xref="polar", yref="polar", x0=-70, y0=-70, x1=70, y1=70, fillcolor="#e74c3c", opacity=0.4, line_width=0, layer="below"),
            ],
        ...
        # --- BLOCO 3: CRONOGRAMA SALVO NA HOME ---
        st.divider()
        st.subheader("🗓️ Seu Cronograma de Execução")
        ...
        # --- BLOCO 4: DETALHAMENTO PÁGINA INTEIRA ---
        st.divider()
